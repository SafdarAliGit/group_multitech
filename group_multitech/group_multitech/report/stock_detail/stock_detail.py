...
def execute(filters=None):
	...
	pmt_wise_qty = get_pmt_wise_qty(conditions,filters)

	pmts = frappe.db.sql("""select pmt from `tabStock Entry` p inner join `tabStock Entry Detail` c on p.name = c.parent
		where p.docstatus = 1 and pmt IS NOT NULL %s group by 1 order by 1"""%(conditions),filters, as_dict=1)
	
	columns = get_column(filters,conditions, pmts)
	...
	for i in master:
		...
		# for d in pmts:
		# 	key = "pmt"+frappe.scrub(str(d.pmt))
		# 	row[key] = 0

		for d in pmt_wise_qty:
...
def get_column(filters,conditions, pmts):
	columns = [
		{
	...
	for d in pmts:
		columns.append({
			"fieldname": "pmt"+frappe.scrub(str(d.pmt)),
			"label": _(d.pmt),
			"fieldtype": "Link",
			"options": "PMT",
			"width": 100
...
def get_pmt_wise_qty(conditions, filters):
	warehouse = get_issue_warehouse(filters)
	data= frappe.db.sql("""select item_code,pmt,SUM(c.qty) as qty from `tabStock Entry` p inner join `tabStock Entry Detail` c on p.name = c.parent
...
def get_conditions(filters):
	...
	if filters.get("team"):
		filters.team = frappe.parse_json(filters.get('team'))
		conditions += " and p.team in %(team)s"
	teams = get_teams()
	if teams:
		filters.teams = tuple(teams)
		conditions += " and p.team in %(teams)s"
	return conditions
